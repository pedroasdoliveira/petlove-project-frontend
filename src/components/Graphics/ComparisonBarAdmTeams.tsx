...
import { BarChartType, UserStorageType } from "types/interfaces";

interface ComparisonBarUserProps {
  teamMapFiltered: UserStorageType[][];
}

const ComparisonBarAdmTeams = ({ teamMapFiltered }: ComparisonBarUserProps) => {
  const comparisonData = teamMapFiltered?.map(
    (dataChart: UserStorageType[]): BarChartType => {
      let teamLength = 0;
      //fazer a media de cada team
      const plus = dataChart?.reduce(
        (acc: number, user: UserStorageType): number => {
